<template>
	<div class="pt-6">
		...
		<form @submit.prevent="edit" class="py-3">
			<input class="block p-2 rounded mb-4 border-2 w-full" type="text" v-model="payload.title" />
...
export default {
	...
	data() {
		return {
			payload: {
				id: this.$route.params.current.id,
				title: this.$route.params.current.title,
				short_description: this.$route.params.current.short_description,
				full_description: this.$route.params.current.full_description
			}
		}
	},

	methods: {
		...mapActions(['editVulnerability']),

		edit() {
			this.editVulnerability(this.payload);
			this.$router.push('/');
		}
	}
}
